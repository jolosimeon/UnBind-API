...
/*	public static void main(String[] args) {
	...
	public Node calcPath(Node from, Node to) {
		...
		while (!nEquals(currNode, to)) {
			...
			System.out.println("I make pili minimum!!!" + currNode.getId());
			removeFrom(open, currNode);
			closed.add(currNode);
			if (nEquals(currNode, to))
				return currNode;
			
			ArrayList<Node> neighbors = Model.getNeighbors(currNode);
			
			System.out.println(" I have kapitbahay!!!" +neighbors.size());
			for (int i = 0; i < neighbors.size(); i++){
				if (!isIn(closed, neighbors.get(i))) {
	...
	public boolean nEquals(Node n1, Node n2) {
		if (Double.compare(n1.getLat(), n2.getLat()) == 0 && Double.compare(n1.getLon(), n2.getLon()) == 0) {
			return true;
		}
		return false;
	}
